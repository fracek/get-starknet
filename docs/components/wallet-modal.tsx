      ...
      <WalletList
        ...
        {({ isSelected, select, ...wallet }) =>
          wallet.state === "available" ? (
            <AvailableWalletButton key={wallet.name} wallet={wallet} isSelected={isSelected} select={select} />
          ) : (
            <UnavailableWalletButton key={wallet.name} wallet={wallet} isSelected={isSelected} select={select} />
          )
        }
      </WalletList>
...
function AvailableWalletButton({
  ...
  isSelected,
  select,
}: { wallet: AvailableWallet, isSelected: boolean, select: () => void }): React.ReactNode {
    const { connect } = useConnect();
  return (
    <WalletButton
        name={wallet.name} icon={wallet.info?.icon} isInstalled={true} isSelected={isSelected} select={select}
...
function UnavailableWalletButton({
  ...
  isSelected,
  select,
}: { wallet: UnavailableWallet, isSelected: boolean, select: () => void }): React.ReactNode {
  return (
    <WalletButton
      name={wallet.name}
...
function WalletButton({
  ...
  select,
  onClick,
}: { icon?: string; name: string; isInstalled: boolean, isSelected: boolean, select: () => void, onClick?: () => void }) {
    const click = () => {
        select();
        onClick?.();
    }
  ...
  return (
    <button
        onClick={() => click()}
        type="button"
        className={"flex flex-row p-4 gap-2 items-center justify-between hover:bg-gray-100/10 cursor-pointer border" + (isSelected ? " border-red-500" : " border-transparent")}
    >
...
function AvailableWalletScreen({
  wallet,
}: { wallet: AvailableWallet }): React.ReactNode {
  return (
    <div>
        Hello
    </div>
...
function UnavailableWalletScreen({
  wallet,
}: { wallet: UnavailableWallet }): React.ReactNode {

  return (
    <div className="flex flex-col items-center gap-4">
        <img alt={wallet.name} className="w-24 h-24" src={wallet.info?.icon ?? ""} />
