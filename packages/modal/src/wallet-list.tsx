...
export type WalletListProps = {
  ...
  children: (wallet: { isSelected: boolean, select: () => void } & MaybeWallet) => React.ReactNode;
};
  ...
  return (
    <div ref={ref} {...props}>
      {sortedWallets.map((wallet) => (
        children?.({ ...wallet, isSelected: selected && walletEq(wallet, selected.wallet), select: () => onSelectedChange({ wallet }) })
      ))}
    </div>
  );
}
